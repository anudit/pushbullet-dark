...
var drawPushChat = function(stream) {
    ...
    var pushes = stream ? filterAndSortPushes(stream) : []
...
var filterAndSortPushes = function(stream) {
    ...
    var pushesFilter = function(push) {
        if (stream.with) {
            ...
            return push.channel_iden == stream.channel.iden
                   || push.channel_tag == stream.channel.tag
        } else if (stream.client) {
            return push.client_iden == stream.client.iden
        } else {
            if (stream.iden == '*') {
